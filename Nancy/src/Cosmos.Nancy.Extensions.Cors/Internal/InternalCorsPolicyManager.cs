...
namespace Cosmos.Nancy.Extensions.Internal
{
    internal static class InternalCorsPolicyManager
    {
        ...
        public static bool IsContainsPolicy(string name) => string.IsNullOrEmpty(name) ? false : _policyMap.ContainsKey(name);

        public static CorsPolicy GetPolicy(string name) => string.IsNullOrEmpty(name) ? null : IsContainsPolicy(name) ? _policyMap[name] : null;
